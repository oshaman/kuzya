...
@section('content')
    <div class="box box-primary">
        <div class="box-header with-border">
            ...
            @if($model->id)
                <a href="{{ route('articles.show', $model->id) }}" class="btn btn-primary">Preview</a>
            @endif
            @include('admin.locale_inputs.lang_menu_switch')
        </div>
        {!! Form::model($model,['url'=>route('articles.'.($model->id ?'update':'store'),$model->id),'method'=>$model->id ?'PUT':'POST','class'=>'']) !!}
        <div class="box-body">
